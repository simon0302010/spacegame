...
use crate::{RES_HEIGHT, RES_WIDTH, collisions::{GROUP_ASTEROID, GROUP_PROJECTILE}, get_high_res_size, player::Player};
...
pub fn manage_projectiles(
    ...
    for ((vel, proj), ent) in velocity_query.iter().zip(entity.iter()) {
        ...
        {
            commands.entity(ent).despawn();
            info!("despawned projecticle with speed {:?} (spawned with {:?})", vel.linvel, proj.initial_velocity);
        }
    }
}